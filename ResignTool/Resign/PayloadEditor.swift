    ...
    func getDylibLinks() -> [String]{
        ...
        if fatArchs.count > 0{
            for arch in fatArchs{
                dylibCmds = machoHandler.getDylibCommand(inFatArch: arch)
                break
            }
        }else{
            dylibCmds = machoHandler.getDylibCommand(inFatArch: nil);
        }
            ...
            if bundleIDInPPF.contains("*") {
                if bundleIDInPPF.elementsEqual("*") {
                    ...
                    if rawBundleID.isMatch(p){
                        ...
                        finalBundleID = bundleIDInPPF.replacingOccurrences(of: "*", with: String.randomString(length: 4).lowercased())
                    }
                }
            } else {
